...
public class Customer implements Updatable {
	...
	public boolean verifyOrder(ArrayList<Ingredient> servedOrder) {
		...
		if(servedOrder.size()!=order.length) {return false;}
		
		for(Ingredient i: servedOrder) {
			boolean check = false;
			for(String s: order) { //Perform a check against each final ingredient, it has to change to true somewhere
				if(i.equals(s)) {
					check = true;
					break;
				}
	...
	public String toString() {
		return "Customer: " + Arrays.toString(order) + ", Remaining Time: " + time;
	}
